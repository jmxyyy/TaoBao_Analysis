package taobao.step05;

import taobao.step05.mapper.ProIdBehaviorMapper;
import taobao.step05.reducer.ProIdBehaviorReducer;
...
public class JobMain extends Configured implements Tool {
    ...
    public int run(String[] strings) throws Exception {
        ...
        job.setMapperClass(ProIdBehaviorMapper.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);
        job.setReducerClass(ProIdBehaviorReducer.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(NullWritable.class);
        job.setOutputFormatClass(TextOutputFormat.class);

        Path out = new Path("/taobao_data_output/Step05");
        ...
        if (flag){
            System.out.println("Step05运⾏成功");
            return 0;
        }
        else{
            System.out.println("Step05运⾏失败");
            return 1;
        }
    }
}
